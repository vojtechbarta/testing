...
The repo workflow [`.github/workflows/azure-backend.yml`](../.github/workflows/azure-backend.yml) deploys a **zip** to **App Service**. If you use **Container Apps**, redeploy the API with `./azure/deploy.sh` after backend changes, or add your own workflow (build image → `az acr push` / `az containerapp update`).
